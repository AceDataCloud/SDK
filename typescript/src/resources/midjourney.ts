/** Midjourney image and video generation resources. */
...
export class Midjourney {
  ...
  async imagine(opts: {
    ...
    const { prompt, action, mask, mode, imageId, timeout, translation, callbackUrl, splitImages, version, hd, quality, styleReference, moodboard, ...rest } = opts;
  ...
  async edits(opts: {
    ...
    const { prompt, action, mask, mode, imageUrl, callbackUrl, splitImages, ...rest } = opts;
    const body: Record<string, unknown> = { ...rest };
    if (prompt !== undefined) body.prompt = prompt;
    if (action !== undefined) body.action = action;
    if (mask !== undefined) body.mask = mask;
