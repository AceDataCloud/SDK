...
export class Kling {
  ...
  async motion(opts: {
    ...
    const { mode, imageUrl, videoUrl, characterOrientation, keepOriginalSound, modelName, watermarkInfo, prompt, callbackUrl, ...rest } = opts;
  ...
  async lipSync(opts: {
    mode: 'audio2video' | 'text2video' | (string & {});
    videoId?: string;
    videoUrl?: string;
    audioUrl?: string;
    audioType?: string;
