...
class CaptchaTasks {
  ...
  async retrieve(opts: { taskId: string; [key: string]: unknown }): Promise<Record<string, unknown>> {
    const { taskId, ...rest } = opts;
    return this.transport.request('POST', '/captcha/tasks', { json: { task_id: taskId, ...rest } });
  }
}
