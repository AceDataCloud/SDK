...
export interface GeminiCompletionsOptions {
  ...
  model: "gemini-3.1-pro" | "gemini-3.0-pro" | "gemini-3.6-flash" | "gemini-3.5-flash" | "gemini-3-flash-preview" | "gemini-2.5-pro" | "gemini-2.5-flash" | "gemini-2.5-flash-lite" | "gemini-2.0-flash" | "gemini-3.1-flash-lite-preview";
...
export class Gemini {
  ...
  async model_generatecontent(options: GeminiModelGeneratecontentOptions): Promise<Record<string, unknown>> {
    ...
    for (const [key, value] of Object.entries(options)) {
      if (!["async", "callbackUrl", "contents", "generationconfig", "maxWait", "pollInterval", "safetysettings", "systeminstruction", "toolconfig", "tools", "wait"].includes(key) && value !== undefined) {
        body[key] = value;
      }
    }
    if (options.callbackUrl !== undefined) body.callback_url = options.callbackUrl;
    return (await this.transport.request('POST', "/v1beta/models/{model}:generateContent", { json: body })) as Record<string, unknown>;
  }
  ...
  async model_streamgeneratecontent(options: GeminiModelStreamgeneratecontentOptions): Promise<Record<string, unknown>> {
    ...
    if (options.callbackUrl !== undefined) body.callback_url = options.callbackUrl;
    return (await this.transport.request('POST', "/v1beta/models/{model}:streamGenerateContent", { json: body })) as Record<string, unknown>;
  }
