...
export interface MidjourneyTasksOptions {
...
export class Midjourney {
  ...
  /** Midjourney Tasks */
  async tasks(options: MidjourneyTasksOptions = {}): Promise<Record<string, unknown>> {
    const body: Record<string, unknown> = {};
    body["action"] = options.action ?? "retrieve";
    if (options.id !== undefined) body["id"] = options.id;
    if (options.traceId !== undefined) body["trace_id"] = options.traceId;
    if (options.ids !== undefined) body["ids"] = options.ids;
    if (options.traceIds !== undefined) body["trace_ids"] = options.traceIds;
    body["offset"] = options.offset ?? 0;
    body["limit"] = options.limit ?? 12;
    for (const [key, value] of Object.entries(options)) {
      if (!["action", "async", "callbackUrl", "id", "ids", "limit", "maxWait", "offset", "pollInterval", "traceId", "traceIds", "wait"].includes(key) && value !== undefined) {
        body[key] = value;
      }
    }
    if (options.callbackUrl !== undefined) body.callback_url = options.callbackUrl;
    return (await this.transport.request('POST', "/midjourney/tasks", { json: body })) as Record<string, unknown>;
  }
