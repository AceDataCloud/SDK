...
func TestMinimaxGenerate(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body["model"] != "MiniMax-H3" || body["duration"] != float64(5) {
			t.Fatalf("unexpected request body: %#v", body)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"task_id":"minimax-task"}`))
	}))
	defer srv.Close()
	...
	handle, err := c.Minimax().Generate(context.Background(), MinimaxGenerateRequest{
		...
		Content:    []MinimaxContent{{Type: MinimaxContentText, Text: "A cat running"}},
	...
	if handle.ID != "minimax-task" {
		t.Fatalf("unexpected task ID: %q", handle.ID)
	}
}
