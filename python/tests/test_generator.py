...
def test_endpoint_uses_manifest_path_when_tasks_are_listed_first():
    spec = {
        "paths": {
            "/wan/tasks": {
                "post": {
    ...
    assert endpoint.summary == "Generate videos"
    assert [p.name for p in endpoint.callable_params] == ["model"]
