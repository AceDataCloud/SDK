"""Dreamina video generation resources."""

from __future__ import annotations

from typing import Any

from acedatacloud._runtime.tasks import AsyncTaskHandle, TaskHandle


class Dreamina:
    """Synchronous Dreamina video client."""

    def __init__(self, transport: Any) -> None:
        self._transport = transport

    def generate(
        self,
        *,
        image_url: str,
        audio_url: str,
        model: str | None = None,
        prompt: str | None = None,
        mask_url: list[str] | None = None,
        callback_url: str | None = None,
        async_: bool | None = None,
        wait: bool = False,
        poll_interval: float = 5.0,
        max_wait: float = 600.0,
        **kwargs: Any,
    ) -> dict[str, Any] | TaskHandle:
        body: dict[str, Any] = {"image_url": image_url, "audio_url": audio_url, **kwargs}
        if model is not None:
            body["model"] = model
        if prompt is not None:
            body["prompt"] = prompt
        if mask_url is not None:
            body["mask_url"] = mask_url
        if callback_url is not None:
            body["callback_url"] = callback_url
        if async_ is not None:
            body["async"] = async_

        result = self._transport.request("POST", "/dreamina/videos", json=body)
        task_id = result.get("task_id")

        if not task_id or (result.get("data") and not wait):
            return result

        handle = TaskHandle(task_id, "/dreamina/tasks", self._transport)
        if wait:
            return handle.wait(poll_interval=poll_interval, max_wait=max_wait)
        return handle


class AsyncDreamina:
    """Async Dreamina video client."""

    def __init__(self, transport: Any) -> None:
        self._transport = transport

    async def generate(
        self,
        *,
        image_url: str,
        audio_url: str,
        model: str | None = None,
        prompt: str | None = None,
        mask_url: list[str] | None = None,
        callback_url: str | None = None,
        async_: bool | None = None,
        wait: bool = False,
        poll_interval: float = 5.0,
        max_wait: float = 600.0,
        **kwargs: Any,
    ) -> dict[str, Any] | AsyncTaskHandle:
        body: dict[str, Any] = {"image_url": image_url, "audio_url": audio_url, **kwargs}
        if model is not None:
            body["model"] = model
        if prompt is not None:
            body["prompt"] = prompt
        if mask_url is not None:
            body["mask_url"] = mask_url
        if callback_url is not None:
            body["callback_url"] = callback_url
        if async_ is not None:
            body["async"] = async_

        result = await self._transport.request("POST", "/dreamina/videos", json=body)
        task_id = result.get("task_id")

        if not task_id or (result.get("data") and not wait):
            return result

        handle = AsyncTaskHandle(task_id, "/dreamina/tasks", self._transport)
        if wait:
            return await handle.wait(poll_interval=poll_interval, max_wait=max_wait)
        return handle
